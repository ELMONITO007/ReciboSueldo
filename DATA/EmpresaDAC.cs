...
namespace Data
{
    public class EmpresaDAC : DataAccessComponent, IRepository<Empresa>
    {
        ...
        public Empresa Load(IDataReader dr)
        {
            ...
            empresa.Id = GetDataValue<int>(dr, "Id_Emprea");
        ...
        {
            const string SQL_STATEMENT = "SELECT * FROM empresa where activo=1 and id_Emprea=@Id ";
        ...
        {
            const string SQL_STATEMENT = "SELECT * FROM empresa where activo=1 and Emprea=@Id ";
            Empresa empresa = null;
        ...
        {
            const string SQL_STATEMENT = "update empresa set empresa=@Empresa, cuit=@Cuit, direccion=@direccion where id_Emprea=@id ";
            var db = DatabaseFactory.CreateDatabase(CONNECTION_NAME);
            using (DbCommand cmd = db.GetSqlStringCommand(SQL_STATEMENT))
            {
