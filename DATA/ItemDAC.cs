...
namespace Data
{
    public class ItemDAC : DataAccessComponent, IRepository<Item>
    {
        ...
        {
            const string SQL_STATEMENT = "insert into Item(porcentaje,item,Tipo,activo)values(@porcentaje,@item,@Tipo,1) ";

            var db = DatabaseFactory.CreateDatabase(CONNECTION_NAME);
            using (DbCommand cmd = db.GetSqlStringCommand(SQL_STATEMENT))
            {
                db.AddInParameter(cmd, "@porcentaje", DbType.Int32, entity.porcentaje);

                db.AddInParameter(cmd, "@item", DbType.String, entity.item);
                db.AddInParameter(cmd, "@Tipo", DbType.Int32, entity.Tipo.Id);
        ...
        public Item Load(IDataReader dr)
        {
            ...
            item.Tipo.Id = GetDataValue<int>(dr, "Tipo");
        ...
        public Item ReadBy(Item entity)
        {
            ...
            using (DbCommand cmd = db.GetSqlStringCommand(SQL_STATEMENT))
            {
                db.AddInParameter(cmd, "@Id", DbType.String, entity.item);
                using (IDataReader dr = db.ExecuteReader(cmd))
                {
                    if (dr.Read())
                    {
